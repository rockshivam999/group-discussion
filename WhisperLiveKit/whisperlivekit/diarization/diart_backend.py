        ...
        with self.segment_lock:
            ...
            if annotation and len(annotation._labels) > 0:
                ...
                for speaker, label in annotation._labels.items():
                    speaker_num = extract_number(speaker)
...
class DiartDiarization:
    ...
    def insert_audio_chunk(self, pcm_array: np.ndarray):
        """Enqueue a PCM chunk coming from the audio processor."""
        if pcm_array is None:
            return
        if self.custom_source:
            if pcm_array.ndim > 1:
                pcm_array = pcm_array.flatten()
            # Diart expects float32 mono audio
            self.custom_source.push_audio(pcm_array.astype(np.float32))

    async def diarize(self):
        """
        Return the current diarization segments collected by the observer.
        This mirrors the interface expected by AudioProcessor.
        """
        # Allow the event loop to schedule other tasks; inference runs in a thread
